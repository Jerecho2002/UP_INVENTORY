    ...
    <div class="fixed inset-0 bg-black/40 flex items-center justify-center p-4">
        <div :class="[
            ...
            <form @submit.prevent="submit">
                <div class="space-y-4">
                    <div v-for="userF in userFields" :key="userF.model">
                        <label class="block text-sm text-[#3B3B3B] font-bold mb-1">
                            {{ userF.label }}
                        </label>

                        <input v-model="form[userF.model === 'user.email' ? 'email' : userF.model]"
                            :type="userF.type || 'text'" :placeholder="userF.placeholder" :class="[
